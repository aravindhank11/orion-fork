...
labels = ['Kernel_Name', 'DRAM_Throughput(%)', 'Duration(ns)', 'Compute(SM)(%)',
          'Block', 'Grid', 'Registers_Per_Thread', 'Static_shmem_per_block',
          'Number_of_threads', 'Number_of_registers']

for kernel in kernels:
    """
    for idx in [-1, -2, -3]:
        if isinstance(kernel[idx], str):
            kernel[idx] = kernel[idx].replace(",", "")
    """
